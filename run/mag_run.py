...
df = pd.read_csv(data_path, sep="\t", header=None, compression="gzip", chunksize=2000)
...
def extract_and_embed_mentions(props, texts, pmids, embeds_df=[]):
    ...
    if len(embeds_df) == 0:
        embeds_df = pd.DataFrame([], columns=["pmid", "property", "mention", "embed"])
