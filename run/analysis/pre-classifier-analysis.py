...
Usage
-----
    uv run python run/analysis/pre-classifier-analysis.py \\
...
def _load_fit_report(path: Path) -> pd.DataFrame:
    report: dict = suthing.FileHandle.load(str(path))
    df = pd.DataFrame(
        {
...
def _load_oov(path: Path) -> pd.DataFrame:
    ...
    elif suffix in {".gz", ".pkl"}:
        obj = suthing.FileHandle.load(str(path))
