...
def test_vrep_pipeline(tokenizer_model_scibert, nlp):
    ...
    ll_tt_stacked, mapping_table = batched_texts_to_vrep([texts], tokenizer, model, indexes_of_interest, layers_spec)

    embs = texts_to_vrep(texts, tokenizer, model, layers_spec, word_mode, max_length=MAX_LENGTH, nlp=nlp)
