...
def test_sentence_ix(nlp):
    s = "TAMs can also secrete in the TME a number of immunosuppressive cytokines, such as IL-6, TGF-β, and IL-10 that are able to suppress CD8+ T-cell function."
    offs = torch.tensor(
        [
            [0, 4],
            [5, 8],
    ...
    word_bnds = get_word_boundaries(s)
    r = map_char_spans_2_token_spans(offs, word_bnds)
    phrase = s[r[0][3][0] : r[0][3][1]]
