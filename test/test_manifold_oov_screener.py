...
def test_evaluate_projection_cv_returns_winner_and_payload() -> None:
    ...
    raw = evaluate_projection_cv(X, y, cfg)
    assert raw is not None
    cv_payload, winner = raw
    assert winner in ("lda", "svm", "rbf")
    assert "winner_f1_mean" in cv_payload
    model, pl = fit_projection_score_model(X, y, cfg, cv_payload_and_winner=raw)
...
def test_fit_projection_returns_none_when_cv_infeasible() -> None:
    X = np.zeros((4, 3), dtype=np.float64)
    y = np.array([0, 0, 0, 1], dtype=np.int64)
    cfg = ManifoldOovScreenerConfig(cv_n_splits=20)
    assert evaluate_projection_cv(X, y, cfg) is None
    assert fit_projection_score_model(X, y, cfg, cv_payload_and_winner=None) == (
        None,
        None,
    )
