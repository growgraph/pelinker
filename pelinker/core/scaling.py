...
~pelinker.clustering.grid.solve_optimal_min_cluster_size` picks the largest MCS that is
statistically tied with the best point *on the grid it was given* — so the choice is
driven by the shape of f(MCS) over a fixed absolute window. Change N, the curve shifts,
the tie region moves, and the window does not.
...
selector is pinned against the fixed ``[min_scale, max_scale)`` grid, and the
