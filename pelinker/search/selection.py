...
from pelinker.clustering.fit import (
...
from pelinker.search.sampling import cap_mentions_per_entity, draw_selection_sample
from pelinker.clustering.transform import TransformArtifacts, score_transform_artifacts
...
def evaluate_selection_sample(
    ...
    *,
    ...
    :func:`~pelinker.reports.schema.n_clusters_at_min_cluster_size` on ``metrics_df``.

    When ``all_metrics_dfs`` is provided, appends this sample's grid ``metrics_df``. Run
    :func:`~pelinker.search.grid_solver.pooled_min_cluster_size_from_metrics_dfs` after all bootstraps
