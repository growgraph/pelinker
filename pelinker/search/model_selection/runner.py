...
from pelinker.search.model_selection.summary import (
...
from pelinker.reports.io import write_model_selection_run_report_json
...
from pelinker.reports.summary import (
...
from pelinker.search.selection import (
...
def run_model_selection(
    ...
    concatenated mention-level vectors via :func:`~pelinker.search.selection.load_selection_frame`
    and per-bootstrap :func:`~pelinker.search.selection.evaluate_selection_sample`.
    ...
    df_table = attach_outer_scores(df_results)
    ...
    df_ranked = attach_outer_scores(df_results)
    if df_ranked.empty:
    ...
    console.print(
        "[dim]Inner MCS: DBCV+ARI (dbcv_ari_geomean, paired 1-SE). "
        "Outer ranking: DBCV+ARI (geomean, per-candidate). "
