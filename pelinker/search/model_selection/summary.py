...
from pelinker.search.model_selection.fusion import combo_key_for_results_row
from pelinker.search.model_selection_checkpoint import (
    DEFAULT_CHECKPOINT_NAME,
    load_checkpoint,
    utc_now_iso,
)
from pelinker.search.grid_solver import solve_pooled_grid_by_combo_from_grid
...
from pelinker.reports.io import write_model_selection_summary_json
...
def build_model_selection_summary_payload(
    ...
    *,
    ...
    if not df_heatmap.empty and "best_score" in df_heatmap.columns:
        scored = attach_outer_scores(df_heatmap)
        try:
            winner = pick_best_row(scored, tie_break_cols=("model", "layer"))
