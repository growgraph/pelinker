...
from pelinker.search.dim_selection.grids import pick_winner_row
from pelinker.search.grid_export import select_grid_points_at_chosen_min_cluster_size
...
DIM_SELECTION_SUMMARY_JSON_SCHEMA = "pelinker.search.dim_selection.summary.v1"
...
def write_results_csv(df: pd.DataFrame, report_path: pathlib.Path) -> pathlib.Path:
    ...
    scored = attach_outer_scores(df) if not df.empty else df
...
def write_dim_heatmaps(df: pd.DataFrame, report_path: pathlib.Path) -> list[str]:
    ...
    written.extend(
        _plot_dim_heatmap(
            ...
            metric_label="Outer DBCV+ARI (geomean)",
...
def write_dim_outer_surface(df: pd.DataFrame, report_path: pathlib.Path) -> list[str]:
    ...
    if rectangular:
        ...
        fig.colorbar(surf, ax=ax, shrink=0.65, label="Outer DBCV+ARI (geomean)")
    else:
    ...
    ax.set_title("Outer DBCV+ARI surface (geomean)")
