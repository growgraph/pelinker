...
2. **Forced noise.** The head is trained only on ``cluster != -1``
   (:func:`~pelinker.linker.entity_head._non_noise_xy`), so it *structurally cannot* emit noise:
...
from pelinker.linker.entity_head import EntityHead
from pelinker.linker.cluster_training import cluster_composition_from_training_frame
...
def cluster_to_entity_map(
    ...
    Reuses :func:`~pelinker.linker.cluster_training.cluster_composition_from_training_frame`
